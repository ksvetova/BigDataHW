...
def MRApproxOutliers(points_rdd, D, M, K):
    ...
    def map_to_cell(point):
        x, y = point
        cell_i = int(x / (D / (2 * (2 ** 0.5))))
        cell_j = int(y / (D / (2 * (2 ** 0.5))))
    ...
    sure_outliers_count = cell_info_rdd.filter(lambda x: x[1][1] > M).count()
    ...
    top_cells = cell_info_rdd.sortBy(lambda x: x[1][0], ascending=False).take(K)
