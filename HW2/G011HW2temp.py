...
def MRApproxOutliers(points_rdd, D, M):
    ...
    cells_rdd = points_rdd.map(map_to_cell).reduceByKey(lambda a, b: a + b).persist()
    ...
    cell_info_rdd = cells_rdd.map(map_to_N3_N7).persist()
...
def SequentialFFT(P, K):
    ...
    distances = list()
    for point in P:
        distances.append(distance(point, S[0]))

    for it in range(1, K):
        farthest_point = P[numpy.argmax(distances)]
        for i in range(len(P)):
            min_d = distance(farthest_point, P[i])
...
def MRFFT(P, K):
    ...
    #    return SequentialFFT(P, K)

    coreset = P.mapPartitions(lambda x: SequentialFFT(list(x), K)).collect()
...
if __name__ == "__main__":
    ...
    inputPoints = sc.textFile(input_file).map(parse_point).repartition(L).persist()
